...
\emph{First of all, I would like to thank the reviewers, who have done a fantastic job and provided very insightful comments for improving the paper. I hope that the authors will take them into serious consideration in the revision.}

\emph{From my personal perspective, this is a very good contribution, which addresses one of the most important topics in the ``citation'' domain, i.e. software citation. I totally agree with the reviewers about the readability of the paper, and I think this paper will deserve publication in PeerJ CS.}

\emph{However, there are some aspects that should be clarified in the revision. In particular:}
...
\emph{This paper argues for the adoption of software citations, basically for the same reasons that data citations have been advocated: Because software packages have become so important in science, they should be appropriately credited and referenced, which can be achieved if we extend the concept of citations in scientific articles.}

\emph{The paper tackles an important problem and provides a very valuable foundation for further discussions and future implementations. In particular, the six software citation principles presented in the beginning are very valuable.}

\emph{While I think this is a very useful paper, I see the following problems with the current version:}
...
\item \textbf{Reviewer comment:}
\emph{line 88: ``which will be presented ...'' > ``which was presented ...''}
...
\niemnote{I think that we should make it clear that unique identifier includes the branch or version, for the first point.}
...
\katznote{we're sorry if this seems weird to the reviewer, but rather than just pointing back to a previous part of the paper, it seems more clear to us to quote the basis by which we are making follow-on comments}
...
\textbf{Our response:}
\niemnote{The formatting will change in the final published paper, so this might not need to be addressed. Or, we can bring in some of the commented statements from above the table}
...
\item \textbf{Reviewer comment:}
\emph{Table 2, page 5: [Comment] This table contains a lot of information, and much food for debate. I was surprised by some omissions, which may mean that I wasn't fully understanding the concepts being presented:\\
\\
...
\katznote{this is a good point - we probably need to change our text a bit}
...
\textbf{Our response:}
Agreed; this change has been been.
