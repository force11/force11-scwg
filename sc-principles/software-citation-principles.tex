...
standard practices for software citation are of concern: \katznote{Are there any more
...
Citation is a a record of software that is important to the research outcome, where provenance is a record of all steps (including software) used to generated particular data within the research process.
...
The ``unique'' in a UID means that it points to a unique, specific software. However, multiple UIDs might point to the same software.  This is not recommended, but is possible.  We strongly recommend that if there is already a UID for a version of software, no additional UID should be
created.  Multiple UIDs can lead to split credit, which does not help anyone.  In particular, we
urge the use of standard DOIs as the UIDs for software. \katznote{This topic is discussed in detail in the `Unique Identification' section of \cite{10.7717/peerj-cs.1}}
