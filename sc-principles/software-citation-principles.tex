...
The principles in this section are written fairly concisely, and further discussed later in this document (in \S\ref{sec:examples}). For example, in this section, we are not defining what software should be cited, but how it should be cited.  In the next section, we discuss what how we think such decisions might be made.

\katznote{maybe should change the language in the principles to be more requirements-like -- e.g. use ``shall'' a lot} \katznote{if the principles are machine readable, that might also imply something about the language used}
...
These software citation principles were originally based on an adaptation of the parallel set of data citation principles~\cite{data-citation-principles}, and then were modified based on working group discussions and the information from the use cases and the related work.
The adaptations and modifications have been made because software, while similar to data in terms of not traditionally having been cited in publications, is also different than data \ldots \katznote{Dan to write a little more here}
...
with the citation of software products.  This is more general than just textual citations of software in a paper, but more general identification of software within the research process. \katznote{other comments on this last sentence are welcome - is this a good enough description of what we mean by software citation?}
...
\item Finding Software - citations enable software to be discovered \LJHnote{and reused}. \katznote{dan to flesh out more} \ssnote{is it worth framing as software(cite \& find)->researcher(credit)->science (reuse) areas?}
\item Reproducibility - citations enable specific software that was
used to be identified, though they are not sufficient to make reproducibility happen \katznote{better language welcome}
\end{itemize}
...
readable for example) this does offer a solution for authors wishing to give explicit instructions to potential citing authors.\dmnote{There is evidence from databases that people actually tend to follow such instructions if they exist: \cite{10.1371/journal.pone.0136631}.} \katznote{adding this ref to the introduction - not sure if it is still needed here too}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discussion and implementation examples}
\label{sec:examples}

\subsection{Discussion of principles}

\paragraph{importance}

\katznote{what about derived software?  What metadata should be preserved from the original? What is done for data today?  should this be in the principles?  tried to add to principle 1 and can discuss in next section. maybe also discuss somewhere else, such as in the list of stuff we recognize but don't address (currently in a GitHub issue)}
\LJHnote{this is analogous to built upon research}

\paragraph{what software to cite}

\katznote{put discussion of what software to cite here -- something about what type of software we think should be cited and shouldn't not, e.g. excel} \LJHnote{sometimes excel could be considered materiel \ldots}
\niemnote{Example of using Excel to simply store and plot data, vs.\ using it for statistical analysis}
...
\paragraph{examples of citation styles}  \scnote{At least four styles (AMS, APA, Chicago, MLA) for
software citation are illustrated with examples in~\cite{lipson2011cite}.}

\paragraph{unique identifiers}  \katznote{recognize that a UID points to a specific software, but multiple UIDs might point to the same software} \LJHnote{not sure that multiple UIDs would be a recommended best practice as  this could cause conflicts} \katznote{LJH: not recommended, but likely will happen anyhow} \katznote{maybe discuss how identifiers are created}

\paragraph{talk about access to software - maybe discuss location}
\LJHnote{recognize that some software is proprietary, not all is open source} 

\subsection{Reproducibility}  \katznote{recognize that more is needed for reproducibility, such as configuration information and runtime environment} \LJHnote{concept of enhance metadata to support this goal?}
